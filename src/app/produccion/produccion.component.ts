...
cadaSegundo()
  {
    ...
    else
    {
      ...
      //Consulta x equipo
      let consulta = "SELECT b.id AS idorden, e.paro_emergencia, e.oee_estado, e.oee_estado_cambio, TIMEDIFF(NOW(), e.oee_estado_cambio) AS tiempoestado, e.oee_turno_actual, e.oee_tripulacion_actual, e.oee_lote_actual, e.oee_parte_actual, e.oee_umbral_alerta, e.nombre as nequipo2, a.estatus AS maquinafuncionando, a.*, b.*, TIME_TO_SEC(TIMEDIFF(NOW(), a.estado_desde)) AS transcurrido, TIME_TO_SEC(TIMEDIFF(NOW(), a.rate_mal_desde)) AS malrate FROM " + this.servicio.rBD() + ".cat_maquinas e LEFT JOIN " + this.servicio.rBD() + ".relacion_maquinas_lecturas a ON e.id = a.equipo LEFT JOIN " + this.servicio.rBD() + ".equipos_objetivo b ON e.oee_lote_actual = b.lote WHERE e.id = " + this.equipoFijo;
      this.verTotal = false;
          ...
          if (+data[0].objetivo > 0)
          {
            ...
            if (+data[0].objetivo <= +data[0].van && data[0].mensaje_fin_enviado!="S" && data[0].mensaje_fin && !this.mostrandoAlerta)
            {
              ...
              this.servicio.consultasBD(campos).subscribe( resp =>
              {
                ...
                width: "420px", panelClass: 'dialogo_atencion', data: { titulo: this.servicio.rTraduccion()[3774], tiempo: 0, mensaje: this.servicio.rTraduccion()[3775], alto: "60", id: 0, accion: 0, botones: 1, boton1STR: this.servicio.rTraduccion()[76], icono1: "in_seleccionado", boton2STR: this.servicio.rTraduccion()[77], icono2: "i_cancelar", icono0: "i_objetivos" }
                });
                respuesta.afterClosed().subscribe(result => {
            ...
            else if (+data[0].objetivo < +data[0].van && data[0].mensaje_sobrecarga && !this.mostrandoAlerta && this.ultimoVan != +data[0].van)
            {
              this.mostrandoAlerta = true;
              const respuesta = this.dialogo.open(DialogoComponent, {
              width: "420px", panelClass: 'dialogo_atencion', data: { titulo: this.servicio.rTraduccion()[3776], tiempo: 0, mensaje: this.servicio.rTraduccion()[3777], alto: "60", id: 0, accion: 0, botones: 1, boton1STR: this.servicio.rTraduccion()[76], icono1: "in_seleccionado", boton2STR: this.servicio.rTraduccion()[77], icono2: "i_cancelar", icono0: "i_objetivos" }
              });
              respuesta.afterClosed().subscribe(result => {
                this.mostrandoAlerta = false;
...
;              }
  ...
  cambiarParametros(accion: number)
  {
    ...
    else if (accion == 3)
    {
      ...
      {
        let sentencia = "SELECT id FROM " + this.servicio.rBD() + ".equipos_objetivo WHERE (equipo = " + this.equipoFijo + " OR equipo = 0) And parte = " + this.detalle.parteActual + " AND estatus = 'A' ORDER BY equipo DESC LIMIT 1;"
        this.rateEquipo = ""
        let campos = {accion: 100, sentencia: sentencia};  
        this.servicio.consultasBD(campos).subscribe( resp =>
        {
      ...
      respuesta.afterClosed().subscribe(result => {
        if (result.accion)
        {
          if (result.accion == 1) 
          {
            ...
            cadAdicional2 = +this.loteActual != +this.detalle.loteActual ? ", oee_lote_cambio = 'S'" : "";
            let sentencia = "UPDATE " + this.servicio.rBD() + ".cat_maquinas SET oee_parte_actual = " + +this.detalle.parteActual + ", oee_lote_actual = " + +this.detalle.loteActual + ",  oee_lote_anterior = " + +this.loteActual + cadAdicional2 + " WHERE id = " + this.equipoFijo + cadAdicional;
            let campos = {accion: 200, sentencia: sentencia};  
            this.servicio.consultasBD(campos).subscribe( resp =>
            {
