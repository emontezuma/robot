 ...
 */    }
    ...
    else if (tabla == 18)
    {
      ...
      sentencia = "SELECT a.*, b.referencia, IFNULL(b.nombre, '" + this.servicio.rTraduccion()[325] + "') AS nombre, IFNULL(CONCAT(c.nombre, ' / ', d.nombre), '" + this.servicio.rTraduccion()[3399] + "') AS nequipo, d.nombre AS nlinea, e.numero, IFNULL(f.nombre, '" + this.servicio.rTraduccion()[1494] + "') AS nturno, IF(a.estatus = 'A', '" + this.servicio.rTraduccion()[1154] + "', '" + this.servicio.rTraduccion()[1155] + "') AS estatus FROM " + this.servicio.rBD() + ".equipos_objetivo a LEFT JOIN " + this.servicio.rBD() + ".cat_partes b ON a.parte = b.id LEFT JOIN " + this.servicio.rBD() + ".cat_maquinas c ON a.equipo = c.id LEFT JOIN " + this.servicio.rBD() + ".cat_lineas d ON c.linea = d.id LEFT JOIN " + this.servicio.rBD() + ".lotes e ON a.lote = e.id LEFT JOIN " + this.servicio.rBD() + ".cat_turnos f ON a.turno = f.id ORDER BY b.nombre;";
...
cambiando(evento: any)
  ...
  asociarTablas(id: number)
  {
    ...
    let whereAdicional = " AND a.id IN (15, 80, 103, 112, 120, 130, 140, 170, 180, 200, 210, 220, 240, 260, 290, 300, 310, 330, 200, 210, 220, 230, 240, 250)";
  ...
  buscarDatos(indice: number, evento: any)
  {
    ...
    else if (indice == 2)
    {
      ...
      //Buscar parte
      this.servicio.activarSpinnerSmall.emit(true);
      let sentencia = "SELECT a.id, c.id AS parte, IF(ISNULL(c.nombre), a.numero, CONCAT(a.numero, ' / ', c.nombre)) AS nombre FROM " + this.servicio.rBD() + ".lotes a LEFT JOIN " + this.servicio.rBD() + ".cat_partes c ON a.parte = c.id WHERE a.id = " + idBuscar;
      let campos = {accion: 100, sentencia: sentencia};  
      this.servicio.consultasBD(campos).subscribe( resp =>
      {
        if (resp.length > 0)
